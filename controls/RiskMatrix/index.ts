// RiskMatrix PCF lifecycle — the LeanToolKit standard pattern:
...
import { RiskMatrixEditor } from "./editor";
import { parseRiskMatrix, serializeRiskMatrix } from "./types";
...
export class RiskMatrix implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  private editor!: RiskMatrixEditor;
  ...
  public init(
    ...
    this.editor = new RiskMatrixEditor(container, {
      onChange: (env, actions) => {
        this.outputJson = serializeRiskMatrix(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this.gate.shouldReload(p)) {
      ...
      const doc = serializeRiskMatrix(envelope);
