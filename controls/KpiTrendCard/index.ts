// KpiTrendCard PCF lifecycle — document-only (no actions channel).

import { IInputs, IOutputs } from "./generated/ManifestTypes";
import { KpiTrendEditor } from "./editor";
import { parseKpiTrend, serializeKpiTrend } from "./types";
...
export class KpiTrendCard implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  private container!: HTMLDivElement;
  private editor!: KpiTrendEditor;
  ...
  public init(
    ...
    this.editor = new KpiTrendEditor(container, {
      onChange: (env) => {
        this.outputJson = serializeKpiTrend(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this.gate.shouldReload(p)) {
      const { envelope } = parseKpiTrend(p.inputJSON?.raw);
      const doc = serializeKpiTrend(envelope);
