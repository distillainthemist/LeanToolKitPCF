// AgendaCard PCF lifecycle — the LeanToolKit standard pattern:
...
import { parseAgenda, serializeAgenda } from "./types";
...
export class AgendaCard implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  private container!: HTMLDivElement;
  private editor!: AgendaEditor;
  ...
  public init(
    ...
    this.editor = new AgendaEditor(container, {
      onChange: (env, actions) => {
        this.outputJson = serializeAgenda(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this.gate.shouldReload(p)) {
      const { envelope, embeddedActions } = parseAgenda(p.inputJSON?.raw);
      ...
      const doc = serializeAgenda(envelope);
