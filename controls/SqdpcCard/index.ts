// SqdpcCard PCF lifecycle — the LeanToolKit standard pattern:
...
export class SqdpcCard implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  private container!: HTMLDivElement;
  private editor!: SqdpcEditor;
  ...
  public init(
    ...
    this.editor = new SqdpcEditor(container, {
      onChange: (env, actions) => {
        this.outputJson = serializeSqdpc(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this.gate.shouldReload(p)) {
      ...
      const doc = serializeSqdpc(envelope);
