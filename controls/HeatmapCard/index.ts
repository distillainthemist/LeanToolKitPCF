// HeatmapCard PCF lifecycle — the LeanToolKit standard pattern:
...
export class HeatmapCard implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  private container!: HTMLDivElement;
  private editor!: HeatmapEditor;
  ...
  public init(
    ...
    this.editor = new HeatmapEditor(container, {
      onChange: (env, actions) => {
        this.outputJson = serializeHeatmap(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this.gate.shouldReload(p)) {
      ...
      const doc = serializeHeatmap(envelope);
