...
export class FiveWhysEditor {
  ...
  private addCause(parentId: string | null): void {
    ...
    const inline = this.inlineActionSection(rootChk.box);
    const depth =
      parentId === null
        ? 1
  ...
  private editCause(cause: CauseNode): void {
    ...
    for (const a of existing) {
      dlg.body.appendChild(this.actionRow(a));
    }
  ...
  /**
   * The action fields (what / who / due), shared between the raise-action
   * dialog and the inline capture that appears the moment a cause is
   * flagged as the root cause.
   */
  private buildActionForm(initial?: LtkAction): {
    el: HTMLElement;
    focus: () => void;
    hasContent: () => boolean;
    ...
    const whoWrap = this.people.length > 0 ? checklist() : el("div");
    ...
    if (this.people.length > 0) {
      for (const person of this.people) {
        ...
        if (currentWho && (currentWho.whoId === person.whoId || currentWho.who === person.who)) {
  ...
  /**
   * The collapsed action section revealed the moment "This is the root
   * cause" is ticked, so capture happens right there in the same dialog.
   */
  private inlineActionSection(rootBox: HTMLInputElement): {
    el: HTMLElement;
    form: ReturnType<FiveWhysEditor["buildActionForm"]>;
  } {
    ...
    section.appendChild(sectionLabel("Action on this root cause"));
    section.appendChild(form.el);
    rootBox.addEventListener("change", () => {
      section.style.display = rootBox.checked ? "flex" : "none";
      if (rootBox.checked) form.focus();
    });
  ...
  /**
   * One action in the edit-cause dialog: a complete/not-complete circle
   * (toggles live — the board behind re-renders, the dialog stays open),
   * the description, and an edit affordance.
   */
  private actionRow(a: LtkAction): HTMLElement {
    const row = el("div", "ltk-fw-action-row");
    const dc = this.doneColor();

    const circle = el("button", "ltk-fw-action-circle");
    circle.type = "button";
    const descEl = el("div", "ltk-fw-action-desc", a.description || a.issue);
    const paint = () => {
    ...
    const main = el("div", "ltk-fw-action-main");
    main.appendChild(descEl);
    main.appendChild(
      el(
        "div",
        "ltk-fw-action-who",
        `${a.assignees[0]?.who ?? "Unassigned"}${a.due ? " · Due " + a.due : ""}`
      )
    );
    main.addEventListener("click", () => this.editAction(a));

    const edit = el("button", "ltk-fw-action-edit", "✎");
    edit.type = "button";
    edit.title = "Edit action";
    edit.addEventListener("click", () => this.editAction(a));
  ...
  private editAction(action: LtkAction): void {
    ...
    const form = this.buildActionForm(action);
    const doneChk = checkItem("Completed");
    doneChk.box.checked = action.status === "done";
    doneChk.wrap.classList.toggle("ltk-check-on", action.status === "done");
    ...
    dlg.body.appendChild(doneChk.wrap);
    form.focus();
  }

  private raiseAction(cause: CauseNode): void {
    const issue = textInput(cause.text, { placeholder: "Issue" });
    const form = this.buildActionForm();
    const dlg = openDialog({
      host: this.root,
      title: "Raise action",
      buttons: [
        { label: "Cancel", kind: "secondary", onClick: () => dlg.close() },
        {
          ...
          kind: "primary",
          onClick: () => {
            if (!form.hasContent() && issue.value.trim() === "") return;
            this.pushAction(cause, issue.value.trim(), form);
            dlg.close();
            this.commitActions();
          },
        },
