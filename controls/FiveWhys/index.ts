// FiveWhys PCF lifecycle — the LeanToolKit standard pattern:
...
import { FiveWhysEditor } from "./editor";
import { parseFiveWhys, serializeFiveWhys } from "./types";
...
export class FiveWhys implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  private container!: HTMLDivElement;
  private editor!: FiveWhysEditor;
  ...
  public init(
    ...
    this.editor = new FiveWhysEditor(container, {
      onChange: (env, actions) => {
        this.outputJson = serializeFiveWhys(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    this.editor.setOptions({ showStatus: p.showStatus?.raw === true });
    ...
    if (this.gate.shouldReload(p)) {
      const { envelope, embeddedActions } = parseFiveWhys(p.inputJSON?.raw);
      ...
      const doc = serializeFiveWhys(envelope);
