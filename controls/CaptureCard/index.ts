// CaptureCard PCF lifecycle — document-only (no actions channel).
...
export class CaptureCard implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  public init(
    ...
    this.editor = new CaptureEditor(container, {
      onChange: (env) => {
        this.outputJson = serializeCapture(env);
  ...
  private applyAll(context: ComponentFramework.Context<IInputs>): void {
    ...
    const rows = parseRows(rawOr(p.rowsJSON, cfg(s, "rowsJSON")));
    this.editor.setConfig(parseColumns(rawOr(p.columnsJSON, cfg(s, "columnsJSON"))), rows.headers, rows.titled);
    ...
    if (this.gate.shouldReload(p)) {
      const { envelope } = parseCapture(p.inputJSON?.raw);
      const doc = serializeCapture(envelope);
