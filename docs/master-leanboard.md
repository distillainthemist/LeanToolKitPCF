...
- **Board screen** — a plain gallery of **Image controls** rendering each
  card's stored `svgExport` (the controls emit it for exactly this), with the
  configured title overlaid as a label. Grid size is just `WrapCount`. Empty
...
| Tile SVG | Multiline (200,000) | `svgExport` at last save — the board tile |
...
- **A — generated defaults (baseline):** every control renders a meaningful
  empty state and emits `svgExport` for it. `tools/tile-defaults.html`
  (served via `tools/pcf-serve.js` against `out/controls/<any>`) renders
  every card type empty and downloads `tile-defaults.json`
  (`{version, tiles: {cardType: svg}}`). Regenerate per release; seed the
  Card Catalog table from it.
- **C — hand-authored:** EmbedCard / CardSettings / MeetingScheduler have no
  `svgExport` by design; the generator includes static placeholders for any
  of them that can sit on a board (EmbedCard).
...
Image = "data:image/svg+xml;utf8," & EncodeUrl(
    Coalesce(ThisItem.cardRow.ben_tilesvg,
