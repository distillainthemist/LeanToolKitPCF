...
on. Snapshots render **inline in the DOM** (never inside an `<img>`), which
sidesteps the WebKit bug where `foreignObject` SVGs render unscaled in image
contexts — so tiles stay at SVG size (~15KB) instead of PNG (~50–250KB).
...
| `gridSize` | `"3x3"` (columns × rows), a bare column count, or empty for a near-square auto fit. Rows grow so every tile always fits; a tile with a taken/invalid `pos` takes the next free slot. |
